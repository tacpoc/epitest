...
namespace EPiServer.Templates.Alloy.Helpers
{
    public static class HtmlHelpers
    {
        ...
        /// <summary>
        /// Renders a ContentArea using a BalanceContentAreaRenderer if ViewData contains one of the tags
        /// defined in Global.ContentAreaTags. If so, content in the content area will be rendered so that
        /// all rows are filled with content.
        /// </summary>
        public static void RenderBalancedContentArea(this HtmlHelper helper, ContentArea contentArea)
        {
            var viewData = helper.ViewContext.ViewData;
            var renderingTag = viewData["tag"] as string;
            if(renderingTag == null || !Global.ContentAreaTagWidths.ContainsKey(renderingTag))
            {
            ...
            var renderer = ServiceLocator.Current.GetInstance<BalancedContentAreaRenderer>();
